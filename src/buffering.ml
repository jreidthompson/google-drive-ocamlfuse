...
module Block = struct
  ...
  let blit_to_arr dest_arr offset block =
    ...
    let src_arr =
      try Bigarray.Array1.sub block.sub_array src_off len
      with Invalid_argument _ as e ->
        Utils.log_with_header
          "Invalid source array (src_off=%d, len=%d, block size=%d, buffer \
           id=%d)\n\
           %!"
          src_off len
    ...
    in
    let dest_arr =
      try Bigarray.Array1.sub dest_arr 0 len
      with Invalid_argument _ as e ->
        Utils.log_with_header
  ...
  let blit_from_arr src_arr offset block =
    ...
    let len = min src_len dest_len in
    let dest_arr =
      try Bigarray.Array1.sub block.sub_array dest_off len
      with Invalid_argument _ as e ->
        Utils.log_with_header
          "Invalid source array (dest_off=%d, len=%d, block size=%d, buffer \
           id=%d)\n\
           %!"
          dest_off len
          (Bigarray.Array1.dim block.sub_array)
    ...
    in
    let src_arr =
      try Bigarray.Array1.sub src_arr 0 len
      with Invalid_argument _ as e ->
        Utils.log_with_header
  ...
  let flush block =
    if block.state = Dirty then
      List.iter
        (fun range ->
          let block_offset =
            Int64.sub range.offset block.start_pos |> Int64.to_int
          in
          Utils.log_with_header
            "BEGIN: Flushing range (content path=%s, start_pos=%Ld, \
             range.offset=%Ld, range.length=%d, block_offset=%d)\n\
             %!"
            block.content_path block.start_pos range.offset range.length
            block_offset;
          let buf =
            Bigarray.Array1.sub block.sub_array block_offset range.length
          in
          Utils.with_out_channel block.content_path (fun ch ->
              let file_descr = Unix.descr_of_out_channel ch in
              Unix.LargeFile.lseek file_descr range.offset Unix.SEEK_SET
              |> ignore;
...
module MemoryBuffers = struct
  ...
  let get_block_index start_pos buffers =
    Int64.div start_pos (Int64.of_int buffers.block_size) |> Int64.to_int

  let get_block_start_pos block_index buffers =
    Int64.mul (Int64.of_int block_index) (Int64.of_int buffers.block_size)

  let remove_block expected_state ((remote_id, block_index) as key) block
      buffers =
    if block.Block.state = expected_state then (
      Utils.log_with_header
  ...
  let release_lru_buffer_if_needed check_condition buffers =
    ...
    let get_total_size_and_lru_block () =
      Hashtbl.fold
        ...
        buffers.blocks (0, null_key, None)
    in
    if Hashtbl.length buffers.blocks > 0 then
      let free_buffers = BufferPool.free_buffers buffers.buffer_pool in
      let pending_requests = BufferPool.pending_requests buffers.buffer_pool in
      if check_condition free_buffers pending_requests then (
        Utils.log_with_header
          "Buffer pool stats: free buffers=%d, pending requests=%d\n%!"
          free_buffers pending_requests;
        let total_size, lru_key, lru_block = get_total_size_and_lru_block () in
        Utils.log_with_header "Memory cache size: %d\n%!" total_size;
        match lru_block with
        | None ->
  ...
  let flush (remote_id, block_index) block_opt buffers =
    let remove_block_from_index () =
      let indexes =
        ...
        | None -> []
        | Some is -> List.filter (fun i -> i <> block_index) is
      in
      Hashtbl.replace buffers.file_block_indexes remote_id indexes
    in
  ...
  let flush_block key buffers =
    let block_opt = Utils.safe_find buffers.blocks key in
    flush key block_opt buffers

  let flush_blocks remote_id buffers =
    Utils.log_with_header "BEGIN: Flushing memory buffers (remote id=%s)\n%!"
      remote_id;
    Utils.with_lock buffers.mutex (fun () ->
        let blocks =
          match Utils.safe_find buffers.file_block_indexes remote_id with
          | None -> []
          | Some is ->
              List.map
                (fun i -> (i, Utils.safe_find buffers.blocks (remote_id, i)))
                is
        in
        List.iter
          ...
          blocks);
    Utils.log_with_header "END: Flushing memory buffers (remote id=%s)\n%!"
      remote_id
  ...
  let get_block block_index remote_id resource_size buffers =
    let add_file_index block_index =
      let indexes =
        ...
        | Some is -> is
      in
      Hashtbl.replace buffers.file_block_indexes remote_id
        (block_index :: indexes)
    in
    ...
    | None ->
          ...
          let size =
            Int64.to_int
              (min
                 ...
                 (Int64.sub resource_size start_pos))
          in
          Block.create start_pos size buffers.mutex buffers.condition
            buffers.buffer_pool
        in
        ...
        Utils.log_with_header
          "END: Acquiring memory buffer (remote id=%s, index=%d, size=%d, \
           buffer id=%d)\n\
           %!"
          remote_id block_index b.Block.size b.Block.buffer.BufferPool.Buffer.id;
        b
    | Some b ->
        Utils.log_with_header
          "Got cached memory buffer (remote id=%s, index=%d, size=%d, buffer \
           id=%d)\n\
           %!"
          remote_id block_index b.Block.size b.Block.buffer.BufferPool.Buffer.id;
        b

  let read_block remote_id offset resource_size fill_array ?dest_arr buffers =
    let get_block block_index =
      get_block block_index remote_id resource_size buffers
    in
    ...
    let wait_for_full_block block =
      while block.Block.state = Block.Writing do
        Utils.log_with_header
          ...
          (Block.state_to_string block.Block.state);
        Condition.wait block.Block.buffer.BufferPool.Buffer.condition
          block.Block.buffer.BufferPool.Buffer.mutex
      done;
      match block.Block.state with
      | Block.Error e ->
          Utils.log_with_header "Streaming error (buffer id=%d, state=%s)\n%!"
            block.Block.buffer.BufferPool.Buffer.id
            (Block.state_to_string block.Block.state);
          raise e
      | _ ->
    ...
    if start_block_index < 0 || offset < 0L then (
      Utils.log_with_header
    ...
    if end_block_index <> start_block_index then
      ...
      if dest_len > 0 then (
        let delta_end_pos = Int64.to_int (Int64.sub end_pos_dest_arr end_pos) in
        let dest_offset = dest_arr_size - dest_len - delta_end_pos in
        let dest_arr =
          Option.map
            (fun arr ->
              try Bigarray.Array1.sub arr dest_offset dest_len
              with Invalid_argument _ as e ->
                Utils.log_with_header
            ...
            dest_arr
        in
        if end_block_index < 0 || src_offset < 0L then (
          Utils.log_with_header
  ...
  let read_ahead read_ahead_buffers remote_id offset resource_size fill_array
      buffers =
    ...
    let rec loop accu requested_buffer_counter =
      ...
      let requested_block_start_pos =
        get_block_start_pos requested_block_index buffers
      in
      if requested_buffer_counter = 0 then SessionM.return accu
        ...
        let read_m =
          ...
          Utils.log_with_header
            "BEGIN: Read ahead resource (remote id=%s, offset=%Ld, \
             requested_block_index=%d, requested_buffer_counter=%d)\n\
             %!"
            remote_id requested_block_start_pos requested_block_index
            requested_buffer_counter;
          read_block remote_id requested_block_start_pos resource_size
            fill_array buffers
          >>= fun () ->
          Utils.log_with_header
        ...
        let accu =
          Utils.with_lock buffers.mutex (fun () ->
              match
                Utils.safe_find buffers.blocks (remote_id, requested_block_index)
              with
              | None -> read_m :: accu
              | Some _ -> accu)
        in
        ...
        | Some bs ->
            let ids =
              List.map
                (fun block_index ->
                  let block =
                    Hashtbl.find buffers.blocks (remote_id, block_index)
                  in
            ...
            Utils.log_with_header
              "END: Releasing %d memory buffers (remote id=%s, buffer ids=%s)\n\
               %!"
  ...
  let write_to_block remote_id content_path src_arr offset buffers =
    ...
    let total_written_bytes =
      Int64.add offset (Int64.of_int (Bigarray.Array1.dim src_arr))
    in
    let resource_size =
      Int64.mul block_size
        (Int64.succ (Int64.div total_written_bytes block_size))
    in
    let write block_index dest_offset src_arr =
      Utils.with_lock buffers.mutex (fun () ->
          let block = get_block block_index remote_id resource_size buffers in
          let bytes = Block.blit_from_arr src_arr dest_offset block in
          let range = { Block.offset = dest_offset; length = bytes } in
    ...
    let end_block_index = get_block_index end_pos buffers in
    if end_block_index < 0 || offset < 0L then (
      Utils.log_with_header
        "Invalid starting block (remote id=%s, start_block_index=%d, \
    ...
    let end_block_bytes =
      if end_block_index <> start_block_index then
        ...
        if src_len > 0 then (
          ...
          let src_arr =
            try Bigarray.Array1.sub src_arr src_offset src_len
            with Invalid_argument _ as e ->
              Utils.log_with_header
                "Invalid starting block array (src_offset=%d, src_len=%d, \
                 ...
                 %!"
                src_offset src_len
                (Bigarray.Array1.dim src_arr);
              raise e
          in
          if end_block_index < 0 || dest_offset < 0L then (
            Utils.log_with_header
              "Invalid ending block (remote id=%s, end_block_index=%d, \
  ...
  let evict_cache buffers =
    let check () =
      Utils.with_lock buffers.mutex (fun () ->
          if buffers.stop_eviction_thread then raise Exit)
    in
    try
      while true do
        for _ = 1 to 10 do
  ...
  let create_eviction_thread buffers = Thread.create evict_cache buffers
