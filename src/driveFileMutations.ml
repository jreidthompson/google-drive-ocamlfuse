...
module type PORTS = sig
  ...
  val write_to_memory_buffers : CacheData.Resource.t -> string -> io_buffer -> int64 -> int
...
module Make (P : PORTS) = struct
  let mark_resource_to_upload runtime resource =
    P.update_cached_resource_state runtime.cache CacheData.Resource.State.ToUpload
  ...
  let truncate runtime path size =
    ...
    P.ensure_local_content resource >>= fun content_path ->
    let remote_id = resource |. CacheData.Resource.remote_id |. GapiLens.option_get in
    ...
    in
    P.update_cached_resource runtime.cache updated_resource;
    let file_size = Int64.sub size (Option.default 0L resource.CacheData.Resource.size) in
    P.shrink_cache ~file_size ();
    if P.file_exists content_path then
      P.truncate_local_file content_path size
    else
      Utils.log_with_header
