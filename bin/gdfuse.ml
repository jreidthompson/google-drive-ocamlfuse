...
let setup_application params =
  ...
  let get_auth_tokens_from_server () =
    ...
    let request_id =
      ...
      if rid = "" then generate_request_id () else rid
    in
    context
  ...
  Utils.log_message "Starting application setup (label=%s, base_dir=%s).\n%!"
    params.filesystem_label params.base_dir;
...
let getattr path =
  ...
  try Drive.get_attr path with e -> handle_exception e "stat" path

let readdir path hnd =
  Utils.log_with_header "readdir %s %d\n%!" path hnd;
  let dir_list =
    try Drive.read_dir path with e -> handle_exception e "readdir" path
  in
...
let opendir path flags =
  ...
  try Drive.opendir path flags with e -> handle_exception e "opendir" path
...
let utime path atime mtime =
  ...
  try Drive.utime path atime mtime with e -> handle_exception e "utime" path

let fopen path flags =
  Utils.log_with_header "fopen %s %s\n%!" path (Utils.flags_to_string flags);
  try Drive.fopen path flags with e -> handle_exception e "fopen" path
  ...
  if !Utils.debug_buffers then
    Utils.log_buffer
      (Printf.sprintf "write %s [%d bytes] %Ld %d" path buf_len offset
         file_descr)
      buf buf_len;
  try Drive.write path buf offset file_descr
  with e -> handle_exception e "write" path

let mknod path mode =
  Utils.log_with_header "mknod %s %o\n%!" path mode;
  try Drive.mknod path mode with e -> handle_exception e "mknod" path

let mkdir path mode =
  Utils.log_with_header "mkdir %s %o\n%!" path mode;
  try Drive.mkdir path mode with e -> handle_exception e "mkdir" path

let unlink path =
  Utils.log_with_header "unlink %s\n%!" path;
  try Drive.unlink path with e -> handle_exception e "unlink" path

let rmdir path =
  Utils.log_with_header "rmdir %s\n%!" path;
  try Drive.rmdir path with e -> handle_exception e "rmdir" path

let rename path new_path =
  Utils.log_with_header "rename %s %s\n%!" path new_path;
  try Drive.rename path new_path with e -> handle_exception e "rename" path

let truncate path size =
  Utils.log_with_header "truncate %s %Ld\n%!" path size;
  try Drive.truncate path size with e -> handle_exception e "truncate" path

let release path flags hnd =
  Utils.log_with_header "release %s %s\n%!" path (Utils.flags_to_string flags);
  try Drive.release path flags hnd with e -> handle_exception e "release" path

let flush path file_descr =
  Utils.log_with_header "flush %s %d\n%!" path file_descr;
  try Drive.flush path file_descr with e -> handle_exception e "flush" path

let fsync path ds file_descr =
  Utils.log_with_header "fsync %s %b %d\n%!" path ds file_descr;
  try Drive.fsync path ds file_descr with e -> handle_exception e "fsync" path

let chmod path mode =
  Utils.log_with_header "chmod %s %o\n%!" path mode;
  try Drive.chmod path mode with e -> handle_exception e "chmod" path

let chown path uid gid =
  Utils.log_with_header "chown %s %d %d\n%!" path uid gid;
  try Drive.chown path uid gid with e -> handle_exception e "chown" path

let getxattr path name =
  Utils.log_with_header "getxattr %s %s\n%!" path name;
  try Drive.get_xattr path name with e -> handle_exception e "getxattr" path

let setxattr path name value xflags =
  Utils.log_with_header "setxattr %s %s %s %s\n%!" path name value
    (Utils.xattr_flags_to_string xflags);
  try Drive.set_xattr path name value xflags
  with e -> handle_exception e "setxattr" path

let listxattr path =
  Utils.log_with_header "listxattr %s\n%!" path;
  try Drive.list_xattr path with e -> handle_exception e "listxattr" path

let removexattr path name =
  Utils.log_with_header "removexattr %s %s\n%!" path name;
  try Drive.remove_xattr path name
  with e -> handle_exception e "removexattr" path

let readlink path =
  Utils.log_with_header "readlink %s\n%!" path;
  try Drive.read_link path with e -> handle_exception e "readlink" path

let symlink target linkpath =
  Utils.log_with_header "symlink %s %s\n%!" target linkpath;
  try Drive.symlink target linkpath
  ...
  if !show_version then
    Printf.printf
      "google-drive-ocamlfuse, version %s\n\
       Copyright (C) 2012-2025 Alessandro Strada\n\
       License MIT\n"
      Config.version
  else
    try
